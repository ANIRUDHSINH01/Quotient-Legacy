...
class ScrimManager(Cog, name="Esports"):
    ...
    async def smanager(self, ctx: Context):
        """
        Contains commands related to Espotive's powerful scrims manager.
        """
    ...
    async def tourney(self, ctx: Context):
        """Create & Manage tournaments with Espotive"""
        ...
        view.add_item(EspotiveView.tricky_invite_button())
    ...
    async def easytag(self, ctx: Context):
        """Commands related to espotive's eztag"""
    ...
    async def set_eztag(self, ctx: Context, *, channel: QuoTextChannel):
        ...
        if count == 1 and not guild.is_premium:
            return await ctx.error(
                f"Upgrade your server to Espotive Premium to setup more than 1 EasyTag channel.\n[Click Me to Purchase]({self.bot.prime_link})"
            )
        ...
        role = discord.utils.get(ctx.guild.roles, name="espotive-tag-ignore")
        if not role:
            role = await ctx.guild.create_role(
                name="espotive-tag-ignore", color=self.bot.color, reason=f"Created by {ctx.author}"
            )
        ...
        embed.description = """
        Unable to mention teammates while registering for scrims or tournaments? Espotive is here for the rescue.

        Use `teammate's ID`, `@teammate_name` or `@teammate's_discord_tag` in your registration format. Espotive will convert that into an actual discord tag.
        """
    ...
    async def tagcheck_set(self, ctx: Context, channel: discord.TextChannel, mentions: int = 4):
        ...
        if count == 1 and not guild.is_premium:
            return await ctx.error(
                f"Upgrade your server to Espotive Premium to setup more than 1 Tagcheck channel.\n[Click Me to Purchase]({self.bot.prime_link})"
            )
