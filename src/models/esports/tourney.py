...
class Tourney(BaseDbModel):
    ...
    name = fields.CharField(max_length=30, default="Espotive-Tourney")
    ...
    def logschan(self) -> Optional[discord.TextChannel]:
        if (g := self.guild) is not None:
            return discord.utils.get(g.text_channels, name="espotive-tourney-logs")
    ...
    async def prompt_selector(ctx: Context, *, tourneys: List["Tourney"] = None, placeholder: str = None):
        ...
        from cogs.esports.views.tourney._select import EspotiveView, TourneySelector
